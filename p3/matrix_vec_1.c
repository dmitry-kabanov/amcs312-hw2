...
void matVecKernel(float **a, float *x, float *restrict y, size_t dim)
{
    size_t i;
    size_t k;
#pragma acc data copyin(a[dim][dim], x[dim]) copy(y[dim])
#pragma acc kernels loop gang(64), vector(512)
...
int main(int argc, char *argv[])
{
    ...
    /* Check */
    for (i = 0; i < dim; ++i) {
        max = fabs(y[i]) > fabs(y_check[i]) ? fabs(y[i]) : fabs(y_check[i]);
        if (fabs(y[i] - y_check[i]) / max > 0.01) {
