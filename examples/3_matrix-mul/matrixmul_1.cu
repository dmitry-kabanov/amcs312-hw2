...
__global__
void matrixmul_1(float* A, float* B, float* C, int dim, int block_size)
{
...
extern "C"
void matrixmul_1_driver(float* dA, float* dB, float* dC, int dim, int block_size)
{
	...
	matrixmul_1<<<dimGrid, dimBlock>>>(dA, dB, dC, dim, block_size);
}