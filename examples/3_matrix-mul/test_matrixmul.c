...
int main(int argc, char* argv[])
{
	if(argc < 6)
	{
		printf("\nUSGAE: %s <matrix-dim> <run-kernel-1> <run-kernel-2> <run-kernel-3> <run-kernel-4>\n", argv[0]);
		exit(-1);
	}
	...
	hc = (float*)malloc(matrix_size * sizeof(float));
	...
	memset(hc, 0, matrix_size * sizeof(float));			// init hc with zeros
	...
	cudaMemcpy(dc, hc, matrix_size * sizeof(float), cudaMemcpyHostToDevice);
	...
	if(1) cublasSgemm('N', 'N', 1, 1, 1, 1.0, da, 1, db, 1, 0.0,  dc, 1);
	...
	for(int i = 0; i < 6; i++)
	{
		...
		for(int j = 0; j < 4; j++)
		{
			...
			switch(j)
			{
				case 0:matrixmul_1_driver(da, db, dc, dim, block_size); break;
				case 1:matrixmul_2_driver(da, db, dc, dim, block_size); break;
				case 2:matrixmul_3_driver(da, db, dc, dim, block_size); break;
				case 3:matrixmul_4_driver(da, db, dc, dim, block_size); break;
				case 4:cublasSgemm('N', 'N', dim, dim, dim, 1.0, da, dim, db, dim, 0.0,  dc, dim); break;
				default:;
			}
			...
			//	cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, dim, dim, dim, 1.0, ha, dim, hb, dim, 0.0, hc, dim);
